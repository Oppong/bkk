...
<div class="container">
         ...
         <div class="flex flex-wrap ">
            @foreach($posts as $post)
                 <div class="  w-full md:w-1/2 pb-4 px-4">
                      ...
                      <p class="pl-2" > <small style="color: #a4b83b;">Posted  {{ $post->created_at->diffForHumans()}} <i class="fa fa-clock-o"></i></small> <small> <i class="fa fa-comments text-muted" > </i>  {{ $post->comments->count() }}</small> </p>
                      <div class="px-2 text-justify text-sm pb-2"><p class=" font-sans text-sm">{!! substr($post->body, 0, 200 ) !!} ...</p></div>
                      <p class="pl-2 text-sm pb-2">Posted In <strong style="color: #a4b83b;">{{ $post->category->name}}</strong> Category</p>
                      <a href="/posts/{{ $post->id }}" class="text-sm pl-2" style="color: #a4b83b;">Readmore &raquo;</a>
                 </div>
            @endforeach
    ...
    <div class=" w-full md:w-1/3 px-2 ">    
         ...
         @foreach($category as $category) 
              ...
              <hr style="border: solid 1px #a4b83b;">
         @endforeach
    </div> {{-- end of w-1/3 --}}
