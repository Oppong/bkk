...
<div class="container">
	 ...
	 <div class="w-full " style="margin-top: 70px;">
	    ...
	    <p class="pl-2" > <small style="color: #a4b83b;">Posted  {{ $post->created_at->diffForHumans()}} <i class="fa fa-clock-o"></i></small>  &nbsp; <span class="text-sm text-grey-dark"><small>By Admin </small> <small> <i class="fa fa-comments text-muted" > </i>  {{ $post->comments->count() }}</small> </span></p>
	    <div class="px-2 text-justify text-sm pb-2 mt-4"><p class=" font-sans text-sm">{!! $post->body!!} </p></div>
	    <p class="pl-2 text-sm pb-2 font-bold"> Posted In <strong style="color: #a4b83b;">{{ $post->category->name}}</strong> Category</p>
	    <hr style="border: solid 1px #eee;">

	      <br>
	         <h5 class="text-sm"><strong style="color: #a4b83b; ">{{ $post->comments->count() }} </strong> Comment(s) for this Post. </h5>
		...
		<h5><strong class="font-sans">Post Comment</strong> </h5>
		  <form action="/comments/{{ $post->id}}" method="Post" class="pt-2">
		      ...
		      <div class="class="mb-4"">
		        <button type="submit" class=" text-white text-sm py-2 px-4 rounded-full mr-3" style="background-color: #a4b83b;">Post Comment</button>
		      </div>
		  </form>
 </div> {{-- end of post comments --}}
{{-- show comments for a post --}}
 <br>
	  <h5 class="text-sm mt-8"><strong style="color: #a4b83b; ">{{ $post->comments->count() }} </strong> Comment(s) for this Post. </h5>
<div class="mt-4 pl-8">
 @foreach($post->comments as $comment)
    <p class="font-sans"> <strong style="color: #a4b83b;  ">{{ $comment->name }}</strong> &nbsp; <span class="text-muted small">{{$comment->created_at->toDayDateTimeString() }}</span> </p>
